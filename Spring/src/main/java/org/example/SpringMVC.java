 ...
 * 调用到{@link AbstractApplicationContext#onRefresh()}有 AbstractApplicationContext#createWebServer()}方法启动Servlet容器
 ...
 * 处理请求过程，DispatcherServlet->HandlerMapping（策略模式）->Chain(Interceptor)->HandlerAdapter（适配器模式）->返回ModelAndView->ViewResolver->Model->通过默认模版引擎Thymeleaf渲染页面
